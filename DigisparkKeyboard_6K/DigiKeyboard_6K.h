 ...
 */
#ifndef __DigiKeyboard_6K_h__
#define __DigiKeyboard_6K_h__
...
class DigiKeyboardDevice : public Print {
 public:
  ...
  void sendKeyPress(byte keyPress, byte modifiers) {
    ...
    memset(&reportBuffer[3], 0, sizeof(reportBuffer) - 3);

    usbSetInterrupt(reportBuffer, sizeof(reportBuffer));
  }

  void sendMultipleKeyPresses(byte keyPresses[], byte modifiers) {
   	while (!usbInterruptIsReady()) {
      // Note: We wait until we can send keyPress
      //       so we know the previous keyPress was
      //       sent.
    ...
    reportBuffer[0] = modifiers;
    memcpy(&reportBuffer[2], keyPresses, sizeof(reportBuffer) - 2);
  ...
  uchar    reportBuffer[BUFFER_SIZE];    // buffer for HID reports [ 1 modifier byte, 1 reserved byte + (len-2) key strokes]
  using Print::write;
};
